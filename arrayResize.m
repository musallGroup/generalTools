function DataOut = arrayResize(DataIn,bin,outtype)
...
% Outtype is the datatype that is return in DataOut. Use 'native' to keep
% consistency with input datatype - using mean is sometimes slower in this
% mode but may help to reduce datasize.
...
if isempty(bin) || isinf(bin) || bin <= 1
    ...
    DataOut = mean(reshape(DataIn,[bin,prod(dSize(1:2))/bin,dSize(3:end)]),1,outtype);
    ...
    DataOut = mean(reshape(DataOut,[bin,prod(dSize(1:2))/bin,dSize(3:end)]),1,outtype);
    DataOut = reshape(DataOut,[dSize(1)/bin,dSize(2),dSize(3:end)]);
