...
- Target location is provided explicitly via --target-root (instead of inserting TAPE_TRANSFER).
- The TAPE_TRANSFER-specific mapping helpers are removed.
- Safety checks ensure:
    (1) target-root is not the same as source
    (2) target-root is not inside source
    (3) source is not inside target-root
- Optional lock behavior: if the target root contains TAPE_TRANSFER_IN_PROGRESS.lock, the script
  forces DRY RUN (to reuse the same coordination mechanism).

Examples (PowerShell)
---------------------
1) Basic:
   python serverTransfer.py "G:\Massive Data Imaging\Proj\Session1" --target-root "E:\"

2) Network drive target:
   python serverTransfer.py "G:\Massive Data Imaging\Proj\Session1" --target-root "\\naskampa\lts\"

3) Dry run:
   python serverTransfer.py "G:\data\run1" --target-root "E:\" --dry-run

4) Force move some file types / keywords:
   python serverTransfer.py "D:\data\run1" --target-root "\\naskampa\lts\" --move-ext .tif .tiff .bin --move-keyword raw video
...
# ----------------------------
# Target mapping for serverTransfer
# ----------------------------
def normalize_target_root(target_root: str) -> PureWindowsPath:
    """
    Normalize a user-provided target root into a PureWindowsPath.
    Accepts drive roots like 'E:\\' or UNC share roots like '\\\\naskampa\\lts\\'.
    """
    p = PureWindowsPath(target_root)
    if not p.parts:
...
def compute_target_path_server(source: str, target_root: str) -> PureWindowsPath:
    """
    Construct target path by replacing the *root* of the source with the user-provided target_root,
    while preserving the remaining relative path.

    Special-case:
      If the source path starts with:
          <drive>\Bpod Local\Data\<Remaining>
      then map the relative path to:
          BpodBehavior\<Remaining>

    Otherwise, keep the same folder structure.
    """
    src = PureWindowsPath(source)
    tgt_root = normalize_target_root(target_root)

    parts = list(src.parts)
    if not parts:
        raise ValueError("Empty source path.")

    # parts[0] is the root ("G:\\") or UNC root ("\\\\server\\share\\")
    rel_parts = parts[1:]

    # --- NEW: Bpod Local\Data -> BpodBehavior mapping ---
    # Example: D:\Bpod Local\Data\427 -> <target_root>\BpodBehavior\427
    if (
        len(rel_parts) >= 2
        and rel_parts[0].lower() == "bpod local"
        and rel_parts[1].lower() == "data"
    ):
        rel_parts = ["BpodBehavior", *rel_parts[2:]]

    return PureWindowsPath(*tgt_root.parts, *rel_parts)



def _norm_for_prefix_check(p: Path) -> str:
    """
...
def validate_source_target_relationship(source_dir: Path, target_dir: Path) -> None:
    """
    Ensure target_dir is not:
      - the same as source_dir
      - inside source_dir
      - and also ensure source_dir is not inside target_dir
    """
    s = _norm_for_prefix_check(source_dir)
    t = _norm_for_prefix_check(target_dir)

    if s == t:
        raise ValueError(f"Target folder must not be the same as source.\n  source={source_dir}\n  target={target_dir}")

    # target inside source?
    if t.startswith(s):
        raise ValueError(
            "Target folder must not be a subfolder of the source.\n"
            f"  source={source_dir}\n  target={target_dir}"
        )
    ...
    if s.startswith(t):
        raise ValueError(
            "Source folder must not be a subfolder of the target.\n"
            f"  source={source_dir}\n  target={target_dir}"
        )
...
def manifest_dir(source_root: Path) -> Path:
    return source_root / ".server_transfer"
    ...
    try:
        ...
        for root, dirs, files in os.walk(source_dir):
            ...
            for fname in files:
                ...
                if ".server_transfer" in src_file.parts:
                    continue
...
def main() -> int:
    ...
    # Safety: forbid same path / nesting in either direction
    try:
        validate_source_target_relationship(src, tgt)
    ...
    if lock_file.exists():
        ...
        print("[LOCK] Target root is locked (transfer in progress).")
    ...
    if forced_by_lock:
        print(f"NOTE: DRY RUN was forced due to {LOCK_FILENAME} in target root")
